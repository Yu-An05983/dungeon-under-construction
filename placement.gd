...
# Shared hologram node
var hologram_node: Node2D = null

# Single tile placement state
...
func _ready():
	...
	# Wait one frame before accepting input so the button click doesn't register
...
func _process(_delta):
	...
	# Cursor hologram always visible unless dragging
	...
	# Start of click — record tile, check if starting on existing conveyor
	if Input.is_action_just_pressed("left click"):
	...
	# While holding — check if dragging
	if Input.is_action_pressed("left click") and last_tile_set:
		var mouse_pos = tilemap.get_local_mouse_position()
		var tile = tilemap.local_to_map(mouse_pos)
	...
	# On release
		...
		if is_dragging and hologram_tiles.size() > 0:
			...
			_place_single_tile()
			# Don't use just_confirmed here so cursor stays visible immediately
		is_dragging = false
...
func _update_cursor_hologram():
	...
	_apply_sprite(cursor_sprite, incoming, current_dir, Color(0.3, 0.7, 1.0, 0.6))
...
func _update_drag_hologram_preview():
	# Clear old sprites except cursor
	...
	for entry in hologram_tiles:
		...
		_apply_sprite(sprite, entry.incoming, entry.dir, Color(0.3, 0.7, 1.0, 0.5))
